...
public class EmailTask {
    ...
    public void remindScore(){
        ...
        // 如果所有部门都已经打分，则不执行
        if(unscoreDeptsForEmail.size() == 0){
            return;
        }
        // 需要发送的邮箱
        ...
        for(String dept:unscoreDeptsForEmail){
            ...
            if((one == null) || StringUtils.isBlank(one.getEmail())){
                throw new BusinessException(ErrorCode.OPERATION_ERROR,one.getUserName()+"不存在或邮箱为空");
            }
            if(StringUtils.isNotBlank(one.getEmail())){
